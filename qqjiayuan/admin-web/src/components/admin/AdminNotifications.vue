  ...
  <div>
    <el-card shadow="never" class="box">
      ...
      <el-table :data="list" v-loading="loading" stripe>
        ...
        <el-table-column prop="nickname" label="收信人" min-width="110" />
...
export default {
  ...
  methods: {
    load () {
      this.loading = true
      api.get('/admin/notifications', { params: { page: this.page, size: this.size } }).then(r => {
        this.loading = false
        if (r.code === 0) { this.list = r.data.list; this.total = r.data.total } else this.$message.error(r.msg)
      })
    },
    del (row) {
      this.$confirm('删除这条通知？', '提示', { type: 'warning' }).then(() => {
        api.delete('/admin/notifications/' + row.id).then(r => {
