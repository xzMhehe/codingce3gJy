...
export default {
  ...
  computed: {
    ...
    // ★ 集结令相关：单次上限由后端 /order/calc 下发（管理端「建筑上限配置」可维护，默认 50），
    //   同时不能超过背包里实际有的数量。接口还没回来时用兜底 50，避免下拉框闪成 0 个。
    orderCapMax () {
  ...
  mounted () {
    ...
    this.loadNotices()
    this.loadCorps()
    // ★ 用户反馈「点了页面刷新就回到首页，刷新后应该还停在刚才那个页面」：
    //   从 URL hash 里的 ezfyPage 参数恢复当前页(cur)，并触发该页的数据加载。
    //   go() 内部已含各页的 loadXxx()，所以走 go() 就够了（基础数据上面已加载）。
    //   ★ 排除依赖临时上下文的页面(wildview/orderpre 要 selCell)，刷新后没有上下文 →
    //     恢复这些页只会显示空面板，不如回首页。
    const noRestore = ['wildview', 'orderpre', 'playerinfo']
    const savedPage = this.pageFromHash()
    if (savedPage && savedPage !== 'home' && noRestore.indexOf(savedPage) < 0) this.go(savedPage)
    else this.syncPageHash('home')
  ...
  methods: {
    ...
    go (t) {
      ...
      // ★ 用户反馈「点了页面刷新就回到首页，刷新后应该还停在刚才那个页面」：
      //   cur 只是内存变量，F5 后重新挂载必然回到默认值 'home'。
      //   这里把当前页写进 URL hash（用 ezfyPage 参数，不动路由路径，避免干扰后退守卫），
      //   mounted 时再读回来恢复。replaceState 不产生历史记录，不影响后退行为。
      this.syncPageHash(t)
    },
    // 从 URL hash 读取上次所在页： #/games/ezfy/cityHome?...&ezfyPage=builds
    pageFromHash () {
      const h = location.hash || ''
      if (h.indexOf('/games/ezfy') < 0) return ''
      const qi = h.indexOf('?')
      if (qi < 0) return ''
      try {
        return new URLSearchParams(h.slice(qi + 1)).get('ezfyPage') || ''
      } catch (e) { return '' }
    },
    // 把当前页写入 URL hash： #/games/ezfy/cityHome?...&ezfyPage=builds
    syncPageHash (t) {
      if (!t) return
      const h = location.hash || ''
      if (h.indexOf('/games/ezfy') < 0) return
      ...
      if (params.get('ezfyPage') === t) return
      params.set('ezfyPage', t)
      try {
        history.replaceState(history.state, '', path + '?' + params.toString())
      } catch (e) { /* 某些环境不允许改 hash，忽略 */ }
    },
    // 节日活动
    loadActivity () {
    ...
    doChatSend () {
      ...
      api.post('/games/ezfy/chat', { content: msg, channel: this.chatChannel }).then(r => {
        if (r.code === 0) {
          ...
          this.chatPage = 1
          this.loadChats()
          // ★ 用户反馈「世界聊天后回首页 发现没出来我刚发的，必须刷新页面才出来」：
          //   首页自己也展示一份聊天(loadHomeChats)，发完必须同步刷新，否则要 F5 才更新。
          this.loadHomeChats()
        } else this.notify(r.msg)
      })
    },
...
.ezfy-page {
  ...
  font-size: 16px;
  color: #333;
  /* ★ 唯一修复: 补上字体族。本页此前从未声明 font-family, 一路继承到浏览器默认(serif),
     于是与全站其它页(style_blue01.css 的无衬线)字体分裂 —— 这就是「字体不一致」的根因。
     取原版 ezfy.css 末尾那句 `body,button,input,select,textarea,h1..h6{font-family:'微软雅黑'}`。
     后续的中文回退只在没装微软雅黑的机器上才用到(Windows 默认都有)。
     ★ 字号/布局一律不改: 原样的 16px 基准 + 16/15/14/13 阶梯是已经调顺眼的版本。 */
...
/* ★ 顶部导航「聊天/邮箱/军情/任务/好友/首页」的间距沿革：
   - 最初 padding: 2px 6px + 全局 a 的 margin: 0 1px = 每词左右各 7px（偏大）
   - 用户「间隔太大 小一些」→ 收到横向 0 + line-height 1.15（过密，用户又反馈「太密集了」）
   - 用户「稍微大一些」→ line-height 定 1.35（行盒 17×1.35≈23px + 4px ≈ 27px）
   - 用户「间隔稍微大一点」→ 横向 padding 定 4px（相邻词距 4+4 = 8px）
   - 用户「又大了 稍微小一点」→ 收到 **3px（相邻词距 6px）← 当前值**
   ★ 纵向: 本规则必须显式写 line-height —— 不写就继承全局 body{line-height:1.5}，
     17px 字下每行盒 25.5px，叠 2px 上下 padding ≈ 34px 高，明显虚胖。
     参考原版 *{line-height:26px}@19px ≈ 1.37 —— **1.35 就是照它来的**。
   ★★ 横向 padding 值【必须】与 .ezfy-subnav a 完全一致，否则「点进二级页间隔变了」。
      历史坑: 二级菜单曾有 2px 5px 而首页那行是 0 → 每词多 10px，6 词多 60px，
      用户实测「点进去的间隔远了」。改一处必须同改另一处。 */
.ezfy-page .top-nav a {
  display: inline-block;
  /* ★ 用户「间隔稍微大一点」→ 0 改 4px；随后「又大了 稍微小一点」→ 收到 3px
     （相邻两词间距 = 3+3 = 6px，原 0 时是 1+1 = 2px 太密）。
...
/* ★ 首链接去掉左侧 padding/margin: 让第一个词「聊天」的文字与下方正文行(公告/新城市…)左对齐。 */
...
/* ★ 用户反馈「资源/军官/军队/科技/城防/统帅 这些页面的间隔保持一致，和首页一样就行，
   点进去的间隔远了」。
   根因: 首页那行二级菜单是裸 .old-line 里的 <a>(靠 .ezfy-page a 的 margin: 0 1px 撑开,
   父块 .old-line 只有 padding: 2px 0), 而这里额外给了 2px 5px → 每个词多 10px 水平内边距,
   6 个词多出 ~60px, 点进去后整条肉眼可见地变宽。
   ★ 现在: 横向 padding 与 .top-nav a 统一为 4px（用户要求间距大一点），
   纵向 line-height 也统一 1.35, 两条导航行高一致。
   ★★ 改 .top-nav a 的 padding 时【必须】同步改这里，否则又会出现「点进去间隔变了」。 */
...
/* ★ 用户「点进去 资源/军官/军队/科技/城防/统帅 左边 和 聊天 的『聊』字对齐」：
   ...
   两者父容器 (.old-line / .top-nav) 左右 padding 都是 0，所以归零后文字左边缘必定对齐。 */
