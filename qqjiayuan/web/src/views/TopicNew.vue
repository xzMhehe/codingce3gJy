<template>
  <div>
    <!-- 面包屑（诺哈 topic_new.asp：社区>新帖） -->
    <div class="bar"><a href="javascript:;" @click="$router.push('/')">社区</a>&gt;新帖<br></div>
...
export default {
  ...
  methods: {
    load () {
      ...
      this.jump = this.page
      api.get('/threads/new', { params: { page: this.page } }).then(r => {
        if (r.code === 0) {
    ...
    go (p) {
      ...
      this.$router.push({ path: '/threads/new', query: { page: p } })
    }
  }
}
</script>
