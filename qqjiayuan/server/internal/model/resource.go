...
type Resource struct {
	...
	File     string `gorm:"type:varchar(120);uniqueIndex;comment:相对 static 的路径或 db/ 上传件名" json:"file"`   // 相对 static 的路径或 db/ 上传件名
	Category string `gorm:"type:varchar(10);index;comment:badge/avatar/game/priv/other" json:"category"` // badge/avatar/game/priv/other
	Name     string `gorm:"type:varchar(30);comment:资源名称（特权名称等）" json:"name"`                            // 资源名称（特权名称等）
	Level    int    `gorm:"default:0;comment:特权等级 1-8" json:"level"`                                     // 特权等级 1-8
	Status   int    `gorm:"default:1;comment:1启用 0停用（停用后不出现在选择器）" json:"status"`                         // 1启用 0停用（停用后不出现在选择器）
	Data     string `gorm:"type:longtext;comment:base64 data URI（库存图片，管理端上传）" json:"-"`                  // base64 data URI（库存图片，管理端上传）
}
