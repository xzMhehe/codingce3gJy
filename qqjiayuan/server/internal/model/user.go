...
type User struct {
	...
	Color        string    `gorm:"type:varchar(200)" json:"color"` // 昵称颜色，情怀功能（可存逗号分隔的逐字颜色序列），普通用户默认蓝色
	NameStart    *time.Time `json:"name_start"`                     // 个性昵称开通时间（复刻 3GQQ name.html/name_buy.asp）
	NameEnd      *time.Time `json:"name_end"`                       // 个性昵称有效期
